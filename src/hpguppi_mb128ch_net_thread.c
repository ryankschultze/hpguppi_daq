 ...
 * in multiple packets in the new S6 128 channel packet format.
 */
        ...
        size_t packet_data_size, int packets_per_block, int packets_per_spectrum, int overlap_packets)
{
    ...
    d->seqnums_per_block = packets_per_block /packets_per_spectrum;
...
/* Return packet index from a pktsock frame that is assumed to contain an S6/MB
 * UDP packet.
 */
...
/* Return channel from a pktsock frame that is assumed to contain an S6/MB UDP
 * packet.
 */
...
/* Write a baseband S6/MB packet into the block. */
        ...
        int ntime_per_block)
{
    ...
    if(block_chan < 0 || block_chan >= obsnchan) {
        ...
        // or the gateware config does not match OBSSCHAN and OBSBW.
        hashpipe_warn("hpguppi_mb128ch_net_thread",
    ...
    int block_time = 512*(seq_num - d->packet_idx);
    if(block_time >= ntime_per_block) {
        // Should "never" happen (already checked above)
        block_time %= ntime_per_block;
    } else if(block_time < 0) {
        hashpipe_error("hpguppi_mb128ch_net_thread", "Block time less than 0! seq_num %lu d->packet_idx %lu", seq_num, d->packet_idx);
        exit(1);
    }

    hpguppi_s6mb_packet_data_copy_from_payload(
...
static int init(hashpipe_thread_args_t *args)
{
    ...
    if (rv!=0 && errno!=EEXIST) {
        hashpipe_error("hpguppi_mb128ch_net_thread", "Error creating control fifo directory");
    ...
    if (rv!=0 && errno!=EEXIST) {
        hashpipe_error("hpguppi_mb128ch_net_thread", "Error creating control fifo");
        return HASHPIPE_ERR_SYS;
    } else if(errno == EEXIST) {
        errno = 0;
    }
    ...
    // Force chperpkt to be 4 (fixed by gateware)
    p_psp->chperpkt =  4;

    // Calculate TBIN from gateware parameters
    tbin = 128.0 / 448e6;
    // Calculate obsnchan from obsbw and tbin
    p_psp->obsnchan = round( tbin * fabs(obsbw) * 1e6);
    ...
    // Force PKTFMT to be "S6MB"
    hputs(st->buf, "PKTFMT", "S6MB");
    hputr8(st->buf, "TBIN", tbin);
    hputs(st->buf, "OBS_MODE", obs_mode);
    hashpipe_status_unlock_safe(st);

    // Force PKTFMT to be "S6"
    strcpy(p_psp->packet_format, "S6MB");
    // Force packet size to be 8+8192 (fixed by gateware)
    p_psp->packet_size = 8+8192;
    ...
    if (rv!=HASHPIPE_OK) {
        hashpipe_error("hpguppi_mb128ch_net_thread", "Error opening pktsock.");
        pthread_exit(NULL);
    }
...
static void *run(hashpipe_thread_args_t * args)
{
    ...
    if (use_parkes_packets) {
        printf("hpguppi_mb128ch_net_thread: Using Parkes UDP packet format.\n");
    ...
    /* Figure out number of packets per block, etc.  Changing number of
    ...
    size_t packet_data_size = p_ps_params->packet_size - 8;
    ...
    hashpipe_warn("hpguppi_mb128ch_net_thread", "block_size %lu", block_size);
    hashpipe_warn("hpguppi_mb128ch_net_thread", "ntime_per_block %lu", ntime_per_block);
    hashpipe_warn("hpguppi_mb128ch_net_thread", "packet_data_size %lu", packet_data_size);
    hashpipe_warn("hpguppi_mb128ch_net_thread", "packets_per_block %u", packets_per_block);
    hashpipe_warn("hpguppi_mb128ch_net_thread", "seqnums_per_block %u", seqnums_per_block);
    ...
    if (baseband_packets) {
        ...
        } else if (overlap_packets != 0) {
            // XXX This is only true for 8-bit, 2-pol data:
            int samples_per_packet = packet_data_size / (nchan/8) / (size_t)4;
            hashpipe_warn("hpguppi_mb128ch_net_thread", "samples_per_packet=%u", samples_per_packet);
    ...
    while (run_threads()) {
        ...
        do {
            ...
            if(rv == -1 && errno != EAGAIN) {
                hashpipe_error("hpguppi_mb128ch_net_thread", "error reading control fifo)");
            } else if(rv > 0) {
                ...
                // Log command
                hashpipe_warn("hpguppi_mb128ch_net_thread", "got %s command", fifo_cmd);
        ...
        // -8 for S6/MB header
        elapsed_bytes += p_ps_params->packet_size - 8;
...
static hashpipe_thread_desc_t mb128ch_net_thread = {
    name: "hpguppi_mb128ch_net_thread",
    skey: "NETSTAT",
    init: init,
    run:  run,
    ibuf_desc: {NULL},
...
{
  register_hashpipe_thread(&mb128ch_net_thread);
}
