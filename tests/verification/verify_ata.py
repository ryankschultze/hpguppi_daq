...
while(True):
    ...
    # TODO: why is the first block missing?
    block_time = (n_block+1) * g_ntimes
    for t in range(g_ntimes):
        if np.all(gd[:, :, t, :] == d[(block_time + t)//NTIME, :, :, (block_time + t)%NTIME, :]):
            good_cnt += g_nants * g_nchans * g_npols
        else:
            print(n_block, t, gd[0, :, t, 0], d[(block_time + t)//NTIME, 0, :, (block_time + t)%NTIME, 0])
